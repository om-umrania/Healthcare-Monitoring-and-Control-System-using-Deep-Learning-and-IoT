...
def run(show_block_schema=True):
    ...
    for count in range(numBlocks):
        ...
        prevHash = '' if len(blockchain) == 0 else blockchain[-1]['hash']

        sensor_type = "ECG"

        # Dynamic patient and doctor info
        patient_info = {
            "name": f"Patient-{random.randint(1000, 9999)}",
            "address": f"Ward-{random.randint(1, 5)}B, Room-{random.randint(100, 500)}",
            "contact": f"+91-{random.randint(70000, 99999)}-{random.randint(10000,99999)}"
        }

        doctor_info = {
            "name": f"Dr. {random.choice(['Amit', 'Ravi', 'Meera', 'Shreya', 'Raj'])} {random.choice(['Sharma', 'Verma', 'Khan', 'Patel', 'Iyer'])}",
            "specialty": random.choice(["Cardiologist", "Neurologist", "Pulmonologist", "Endocrinologist"]),
            "contact": f"doctor{random.randint(1,999)}@hospital.com"
        }
        ...
        # Swarm-Based Trust Score Calculation with learnable or adaptive weights
        ...
        trust_score = 0.25 * energy_weight + 0.35 * stake_weight + 0.4 * pdr_weight
        trust_scores.append(trust_score)

        # Step 3: Probabilistic Block Addition using sigmoid for smooth mapping
        acceptance_probability = sigmoid(10 * (trust_score - 0.5))
    ...
    trust_threshold = np.percentile(trust_scores, np.random.uniform(60, 80))
    qos_threshold = np.percentile(block_delays, np.random.uniform(60, 80))

    for i in range(len(trust_scores)):
        numerator = abs(trust_scores[i] - trust_threshold)
        denominator = max(trust_scores[i], trust_threshold, 1e-6)
        similarity = 100 * (1 - (numerator / denominator))

        adaptive_threshold = 45 + 5 * np.sin(i / 100)  # smoothly adaptive threshold
        predicted_class = "secure" if similarity >= adaptive_threshold else "faulty"
    ...
    print("\n--- Block Trust Classification Evaluation (Swarm + Fuzzy + Probabilistic with Sigmoid) ---")
    ...
    plt.title('Classification Metrics - Enhanced PoS Consensus (Swarm + Fuzzy + Probabilistic with Sigmoid)')
    plt.ylabel('Score')
    plt.ylim(0, 1)
    plt.grid(axis='y')
    plt.show()
